...
from daisyfl.utils.logger import ERROR, INFO, WARNING, log
...
class TaskManager:
    ...
    def receive_task(self, task_config: Dict, parameters: Optional[Parameters] = None) -> Tuple[Parameters, Report]:
    ...
    def parse_task_spec(
        ...
        if not task_config.__contains__(MIN_WAITING_TIME_MASTER):
            ...
            if not task_config.__contains__(MIN_WAITING_TIME_ZONE) or not task_config.__contains__(ZONE_COMM_FREQUENCY):
                ...
                task_config[MIN_WAITING_TIME_MASTER] = (
                    task_config[MIN_WAITING_TIME_ZONE] * task_config[ZONE_COMM_FREQUENCY]
                )
        ...
        task_config[CURRENT_ROUND] = (
            task_config[CURRENT_ROUND_MASTER] * task_config[ZONE_COMM_FREQUENCY] + task_config[CURRENT_ROUND_ZONE]
        )
    ...
    def assign_task(
        ...
        if ("parameters" not in locals()) or ("report" not in locals()):
            log(
                ERROR,
                "Zone receive a reduncdant subtask. Please check the task configuration. %s",
                task_config,
            )
