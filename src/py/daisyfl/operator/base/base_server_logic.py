...
# ==============================================================================
"""Base server logic for standard synchronous federated learning rounds."""
from typing import Optional, Tuple

from daisyfl.common import METRICS, MIN_WAITING_TIME, TID, Parameters, Report, Task
    ...
    def fit_round(
        ...
        if self.strategy.wait_fit(subtask_status=subtask_status, min_waiting_time=task.config[MIN_WAITING_TIME]):
            ...
            results = self.communicator.get_results(subtask_id) + self.communicator.get_results_roaming(
                tid=task.config[TID], is_fit=True
            )
