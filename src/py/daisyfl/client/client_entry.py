...
# ==============================================================================
"""Client entry module for managing client-to-zone gRPC connections and message handling."""
import sys
import threading
import uuid
...
from daisyfl.utils.logger import DEBUG, ERROR, INFO, WARNING, log
...
class ClientEntryStatus(Enum):
    """Status codes representing the current state of a ClientEntry."""
...
class ClientEntry:
    ...
    def __init__(
        ...
        """Initialize ClientEntry and start the client-to-zone connector thread."""
    ...
    @staticmethod
    def _check_metadata(metadata: Optional[Tuple] = None) -> Tuple:
        if metadata is None:
    ...
    # APIs
    def get_client_entry_status_code(
        self,
    ) -> int:
        """Return the integer status code of this ClientEntry."""
        return self._status.value

    def get_client_status(
        self,
    ) -> Optional[ClientMessage.ClientStatus]:
        ...
        if status_code == ClientEntryStatus.HANDLING_AND_SENDING.value:
    ...
    def get_anchor_fn(
        ...
        """Return the get_anchor function used by ClientOperator for roaming."""
