...
class ZoneServiceServicer(transport_pb2_grpc.DaisyServiceServicer):
    ...
    def __init__(
        ...
        """Initialize ZoneServiceServicer with a client manager and server address."""
    ...
    def Join(
        ...
        """Method will be invoked by each client_entry or zone_service_servicer which participates in the network."""
        # process Iterator
        ...
        client_message, success = self.get_client_message(
            client_message_iterator=client_timeout_iterator, context=context, timeout=3
        )
        if not success:
            return
        ...
        if field == "client_status":
            ...
            if metadata_dict.__contains__(HANDOVER):
                # handover
                yield from self.handle_roaming_connection(
                    client_timeout_iterator=client_timeout_iterator, context=context
                )
            else:
        ...
        # anchor is notified by another zone
            ...
            yield from self.handle_roaming_request(
                client_timeout_iterator=client_timeout_iterator,
                context=context,
            )
        # shutdown request
        elif field == "shutdown":
    ...
    def handle_roaming_connection(
        ...
        try:
            ...
            with grpc_connection(
                ...
                if upload_success:
                    if server_waiting:
                        ...
                        if field == "fit_res":
                            ...
                            error_code = daisyfl_serde.evaluate_res_from_proto(
                ...
                rts_msg = ClientMessage(
                    roaming_termination_signal=daisyfl_serde.roaming_termination_signal_to_proto(rts)
                )
                send(rts_msg)
    ...
    def handle_roaming_request(
        ...
        """Method to handle a temporary connection from another ZoneServiceServicer.

        Checks some information related to roaming.
        """
