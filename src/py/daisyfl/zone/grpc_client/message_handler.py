...
from daisyfl.common import CID, CURRENT_ROUND, METRICS, ErrorCode, EvaluateRes, FitRes, Status
...
def _fit(fit_msg: ServerMessage.FitIns, task_manager: TaskManager) -> ClientMessage:
    ...
    if not report.config.__contains__(CID):
        log(WARNING, 'Configuration of FitRes doesn\'t contain "CID". "CID" in FitIns will be used in FitRes.')
        report.config[CID] = fit_ins.config[CID]

    # Serialize fit result
    fit_res_proto = daisyfl_serde.fit_res_to_proto(
        FitRes(
...
def _evaluate(evaluate_msg: ServerMessage.EvaluateIns, task_manager: TaskManager) -> ClientMessage:
    ...
    if not report.config.__contains__(CID):
        ...
        report.config[CID] = evaluate_ins.config[CID]

    # Serialize evaluate result
    evaluate_res_proto = daisyfl_serde.evaluate_res_to_proto(
        EvaluateRes(
            status=Status(error_code=ErrorCode.OK, message="Success"),
            config=report.config,
        )
